...
export default async function ScanPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const outlet = await prisma.outlet.findUnique({ where: { qrToken: token } });
  ...
  if (!outlet || outlet.tenantId !== session.tenantId) {
    return (
      <main className="mx-auto flex w-full max-w-sm flex-1 flex-col justify-center gap-3 px-6 py-16 text-center">
        ...
        <p className="text-sm text-stone-600">This QR code doesn&apos;t match a ShakeWallah outlet.</p>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto flex w-full max-w-sm flex-1 flex-col gap-6 px-6 py-10">
      <div>
        <p className="text-xs font-medium uppercase tracking-wide text-rose-700">Claim points</p>
